...
// Removes the "Cannot update component" warning
// ============================================
...
function LoginContent() {
  ...
  // ✅ FIX: Move redirect logic into useEffect with loop prevention
  useEffect(() => {
    if (!authLoading && isAuthenticated && !hasAttemptedRedirect.current) {
      hasAttemptedRedirect.current = true;

      console.log('🔄 Login page: User is authenticated, redirecting...', { isAdmin, redirect });

      const targetUrl = isAdmin ? "/admin" : redirect;
      console.log('🎯 Redirecting to:', targetUrl);

      // Use window.location.href for reliable redirect (no fallback needed)
      window.location.href = targetUrl;
    }
  }, [isAuthenticated, isAdmin, authLoading, redirect]);
  ...
  if (isAuthenticated) {
    console.log('✅ Login page: Authenticated, showing redirect spinner...');
    return (
      <div className="min-h-screen bg-dark-950 flex items-center justify-center">
